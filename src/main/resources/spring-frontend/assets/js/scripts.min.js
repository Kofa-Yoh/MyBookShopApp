    ...
    $(function () {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    (book.discount > 0 ? '<div class="Card-sale">' + book.discount + '% скидка</div>' : '') +
                    ...
                    (book.discount > 0 ? '<span class="Card-priceOld">₽' + book.price + '</span>\n' : '') + '                       <span class="Card-price">₽' + book.discountPrice + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                    $('[data-refreshshow]').on('click change', function (e) {
                        ...
                        switch (data) {
                            ...
                            case'popular':
                                ...
                                break;
                            case'genre':
                                getData('/genres/page/' + $this.data('refreshid'), {
                                    offset: function(){
                                        var oldOffset = $this.data('refreshoffset');
                                        if(oldOffset < 1){
                                            $this.data('refreshoffset', ++oldOffset);
                                ...
                                break;
                            case'author':
                                getData('/books/author/' + $this.data('refreshid') + '/page', {
                                    offset: function(){
                                        var oldOffset = $this.data('refreshoffset');
                                        if(oldOffset < 1){
                                            $this.data('refreshoffset', ++oldOffset);
                                ...
                                break;
                            case'tag':
                                getData('/tags/page/' + $this.data('refreshid'), {
                                    offset: function(){
                                        var oldOffset = $this.data('refreshoffset');
                                        if(oldOffset < 1){
                                            $this.data('refreshoffset', ++oldOffset);
        ...
        var CartBlock = function () {
            return {
                init: function () {
                    var cartCount = 0;
                    var cartContents = $.cookie("cartContents");
                    if (cartContents != null && cartContents != "") {
                        while(cartContents.charAt(0) === '/')
                        {
                            cartContents = cartContents.substring(1);
                        }
                        cartCount = cartContents.split("/").length;
                    }
                    $('.CartBlock-cart .CartBlock-amount').text(cartCount);

                    var postponedCount = 0;
                    var postponedContents = $.cookie("postponedContents");
                    if (postponedContents != null && postponedContents != "") {
                        while(postponedContents.charAt(0) === '/')
                        {
                            postponedContents = postponedContents.substring(1);
                        }
                        postponedCount = postponedContents.split("/").length;
                    }
                    $('.CartBlock-postponed .CartBlock-amount').text(postponedCount);
                }
            };
        };
        CartBlock().init();
            ...
            return {
                init: function () {
                    ...
                    $('.Comments-addComment form').on('submit', function (e) {
                        ...
                        if (!error) {
                            Login().postData('/books/bookReview', {
                    ...
                    $('[data-likeid]').on('click', function (e) {
                        ...
                        Login().postData('/books/rateBookReview', data, function (result) {
                ...
                init: function () {
                            ...
                            postData('/approveContact', data, function (result) {
                                if (result.result) {
                                    ...
                                    if (result) {
                                        ...
                                        $row.find('.form-input_code').addClass('form-input_error')
                                            .after('<div class="form-error">' + 'Wrong code or phone number' + '</div>');
                                    } else {
                    ...
                    $sendButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            if (!error) {
                                postData('/checkContact', $this.data('send'), function (result) {
                                    if (result.result === 'false') {
                                        $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">Контакт не найден. Проверьте правильность введенных данных</div>');
                                    } else {
            ...
            return {
                init: function () {
                    ...
                    $('input[name="mail"], input[name="phone"]').on('change', function () {
                        var $this = $(this);
                        var $confirm = $this.next('.Profile-btn_confirm')
                        $this.removeClass('form-input_error')
                            .removeClass('approved')
                            .addClass('no-approved');
                        $this.next('.form-info').remove();
                        $confirm.find('.form-group_key').hide();
                        $confirm.find('.form-group_btn .btn').removeAttr('disabled')
                            .addClass('btn_check');
                        $confirm.find('.form-info').remove();
                        $confirm.show();
                        $form.find('.Profile-success').remove();
                        $form.find('.Profile-error').remove();
                    });

                    $('#submitProfileMail, #submitProfilePhone').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group');
                        if ($this.attr('id') === 'submitProfileMail') {
                            var data = {contact: $row.find('.form-input_mail').val()};
                            var endpoint = '/requestEmailConfirmation';
                        } else if ($this.attr('id') === 'submitProfilePhone') {
                            var data = {contact: $row.find('.form-input_phone').val()};
                            var endpoint = '/requestContactConfirmation';
                        }
                        if ($this.hasClass('btn_check')) {
                            Login().postData('/checkContact', data, function (result) {
                                if(result.result === 'false') {
                                    Login().postData(endpoint, data, function (result) {
                                        if (result.result === 'true') {
                                            $this.after('<div class="form-info">Вам направлен код подтверждения</div>')
                                            $this.attr('disabled', 'disabled')
                                                .removeClass('btn_check');
                                            $row.find('.form-group_key').show();
                                            $row.addClass('form-group_confirm');
                                            $row.find('.form-error').remove();
                                        } else {
                                            $row.find('.form-input[type="text"]').addClass('form-input_error')
                                                .after('<div class="form-error">' + result.error + '</div>');
                                        }
                                    })
                                } else {
                                    $row.find('.form-input[type="text"]').addClass('form-input_error')
                                        .after('<div class="form-error">Данный контакт уже используется</div>');
                                }
                            })
                        } else if ($this.hasClass('btn_approve')) {
                            Login().postData('/approveContact', data, function (result) {
                                if (result.result === 'true') {
                                    $row.find('.form-input[type="text"]')
                                        .removeClass('no-approved')
                                        .addClass('approved')
                                        .after('<div class="form-info">Контакт подтвержден</div>');
                                } else {
                                    $row.find('.form-input[type="text"]')
                                        .addClass('form-input_error')
                                        .after('<div class="form-error">Проверьте правильность введенного кода</div>');
                                }
                                $row.find('.Profile-btn_confirm').hide();
                            })
                        }
                        $form.find('.Profile-success').remove();
                        $form.find('.Profile-error').remove();
                    ...
                    $('.form-input_profile_code').on('change', function() {
                        var $btn = $(this).closest('.Profile-btn_confirm').find('.form-group_btn  .btn');
                        $btn.removeAttr('disabled')
                            .addClass('btn_approve');
                        $btn.next('.form-info').remove();
                    });

                    $('.btn_profile_save').on('click', function(e) {
                        var $this = $(this);
                        if ($form.find('input').hasClass('no-approved')) {
                            e.preventDefault();
                            $form.find('.no-approved')
                                .after('<div class="Profile-error">Для изменения контактов необходимо подтверждение</div>');
                        }
                    });
            ...
            return {
                init: function () {
                    $('form.Topup').on('submit', function (e) {
                        $(this).find('.Topup-error').remove();
                        var $this = $(this);
                        e.preventDefault();
                        Login().postData('/topup', {
                            hash: $this.attr('data-sendHash'),
                            ...
                            if (result.error === null || result.error === '') {
                                ...
                                $('.Topup-wrap').append('<div class="Topup-error">' + result.error + '</div>')
                            }
                        })
                    });
                }
        ...
        var Transactions = function () {
            var transactionTemplate = function (transaction) {
                return '<tr class=' + (transaction.status == 1 ? 'Transactions_textSuccess' : 'Transactions_textFail' ) + '>\n' +
                    '<td>' + transaction.time + '</td>\n' +
                    '<td><span>\n' +
                    '   <strong>' + transaction.sum + '</strong></span>\n' +
                    '<td>' + transaction.description +
                        (transaction.book != null ? ' <a href=/books/' + transaction.book.slug + '>' + transaction.book.title + '</a>' : '') +
                        (transaction.status == 0 ? ' - <strong>Отмена</strong>' : '') +
                    '</td>\n' +
                    '</tr>';
            }
                ...
                init: function () {
                    $('.Transactions-get').on('click', function (e) {
                        ...
                        getData('/transactions', {
                            ...
                            offset: function(){
                                var oldOffset = $this.data('transactionoffset');
                                $this.data('transactionoffset', ++oldOffset);
                                return $this.data('transactionoffset');
                            },
                            limit: $this.data('transactionlimit')
                        })
                    });
